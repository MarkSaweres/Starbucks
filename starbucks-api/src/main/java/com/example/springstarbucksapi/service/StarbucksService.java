...
//        orders.clear();
    ...
    public StarbucksOrder newOrder(String regid, StarbucksOrder order) throws ResponseStatusException {
        ...
        Optional<StarbucksOrder> activeOptional  = ordersRepository.findByRegister(regid);
        if (activeOptional.isPresent()) {
            StarbucksOrder active = activeOptional.get();
...
//        orders.put(regid, new_order);
    ...
    public StarbucksOrder getActiveOrder(String regid) {
        return ordersRepository.findByRegister(regid).orElseThrow(() -> new ResponseStatusException(HttpStatus.BAD_REQUEST, "Order Not Found!"));
    }

    /* Clear Active Order */
    public void clearActiveOrder(String regid) {
        StarbucksOrder starbucksOrder = ordersRepository.findByRegister(regid).get();
        ordersRepository.delete(starbucksOrder);
    }
    ...
    public StarbucksCard processOrder(String regid, String cardnum) throws ResponseStatusException {
        ...
        StarbucksOrder active = ordersRepository.findByRegister(regid).get();
        if (active == null) {
